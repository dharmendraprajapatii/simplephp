
$(document).ready(function(){
    var firebaseConfig={
    "api_key":"AIzaSyD3P4c2dNlJ3Un-xHejUUGoMTwb_qQ8jbw",
    "type": "service_account",
    "project_id": "locsaleapplication",
    "private_key_id": "01d372832bc264fbf1e4265822c19fc5579e07dc",
    "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
    "client_email": "firebase-adminsdk-6hjf0@locsaleapplication.iam.gserviceaccount.com",
    "client_id": "104206983025500029508",
    "auth_uri": "https://accounts.google.com/o/oauth2/auth",
    "token_uri": "https://oauth2.googleapis.com/token",
    "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
    "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-6hjf0%40locsaleapplication.iam.gserviceaccount.com",
    "databaseURL": "https://locsaleapplication-default-rtdb.firebaseio.com",
    };
    firebase.initializeApp(firebaseConfig);

    var database=firebase.database();
    var userId=1;
    
    database.ref('Users').on('value', function(snapshot){
        var userData=snapshot.val();
        var html=[];
        html.push(' <option id="first" value="Software" selected disabled>Please Select Category</option>')
        $.each(userData, function(index, value){
            
          if (value.type == 1 ) {
             
              html.push('\
              <option class="category" data-name="'+value.business_sub_category+'">'+value.business_field+':'+value.business_sub_category+'</option>\
              ');
             
            
          }
          
        });
        $('#categoryData').html(html);

        jQuery("#categoryData").change( function() {
            var selectedOption = jQuery(this).find(":selected").attr("data-name");

            var cData=selectedOption;
            var userId1=1;
            var html1=[];
            $.each(userData, function(index, value){
                if (value.business_sub_category == cData) {
                    html1.push('<tr>\
                    <td>'+userId1++ +'</td>\
                    <td>'+value.business_name+'</td>\
                    <td>'+value.business_field+'</td>\
                    <td>'+cData +'</td>\
                    </tr>');
                 
                    
                }
                
            });
            $('#allDataCategory').html(html1);
        } );    
        
    }); 
     
   
});




// var userXData;

// $(document).ready(function(){
//     var firebaseConfig={
//     "api_key":"AIzaSyD3P4c2dNlJ3Un-xHejUUGoMTwb_qQ8jbw",
//     "type": "service_account",
//     "project_id": "locsaleapplication",
//     "private_key_id": "01d372832bc264fbf1e4265822c19fc5579e07dc",
//     "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
//     "client_email": "firebase-adminsdk-6hjf0@locsaleapplication.iam.gserviceaccount.com",
//     "client_id": "104206983025500029508",
//     "auth_uri": "https://accounts.google.com/o/oauth2/auth",
//     "token_uri": "https://oauth2.googleapis.com/token",
//     "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
//     "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-6hjf0%40locsaleapplication.iam.gserviceaccount.com",
//     "databaseURL": "https://locsaleapplication-default-rtdb.firebaseio.com",
//     };
//     firebase.initializeApp(firebaseConfig);

//     var database=firebase.database();
//     var userId=1;
    
//     database.ref('Users').on('value', function(snapshot){
//         var userData=snapshot.val();
//         userXData = userData;
//         var html=[];
//         html.push(' <option id="first" value="Software" selected disabled>Please Select Category</option>');
//         $.each(userData, function(index, value){
            
//           if (value.type == 1 ) {
             
//               html.push('\
//               <option class="category" onclick="getCategoriesData(this)" data-name="'+value.business_sub_category+'">'+value.business_field+':'+value.business_sub_category+'</option>\
//               ');
             
            
//           }   
          
//         });
//         $('#categoryData').html(html);

            
        
//     }); 
     
   
// });

// function getCategoriesData(node) {

     
//         alert('hii');
//         var cData=$(node).attr('data-name');
//         var userId1=1;
//         var html1=[];
//         $.each(userXData, function(index, value){
//             if (value.business_sub_category == cData) {
//                 html1.push('<tr>\
//                 <td>'+userId1++ +'</td>\
//                 <td>'+value.business_name+'</td>\
//                 <td>'+value.business_field+'</td>\
//                 <td>'+cData +'</td>\
//                 </tr>');
             
                
//             }
            
//         });
//         setTimeout(
//             function() 
//             {
//                 $('#allDataCategory').html(html1);

//                }, 1500); 
       
  
// }

function test() {
    alert("function test");
}